...
# إعداد Gemini (تم استخدام الإصدار الأحدث لتفادي خطأ 404)
...
EXCLUDED = [
    "زواج", "مسيار", "خطابة", "بنت", "شاب", "غرفة", "غرفه", "سكن", "اشارك", "للبيع", "حراج",
    "متواجد", "متاح", "شغال", "تحميل", "يوجد لدينا", "خدمة توصيل", "أسعارنا", "جاهز"
]
INTENT_WORDS = ["مشوار", "توصيل", "سواق", "كابتن", "سيارة", "يوصلني", "يوديني", "ابغى", "ابي"]
...
async def is_real_delivery_order(text):
    ...
    - إذا كانت طلب توصيل أو مشوار: رد بكلمة "YES" فقط.
    - إذا كانت إعلان سائق، طلب سكن، زواج، أو أي شيء آخر: رد بكلمة "NO" فقط.
...
async def start_radar():
    ...
    print(f"✅ الرادار الذكي (AI) بدأ العمل الآن... الحساب: {me.first_name}")
    
    monitored_chats = []
    async for dialog in user_app.get_dialogs(limit=50):
        if str(dialog.chat.type) in ["ChatType.GROUP", "ChatType.SUPERGROUP"]:
            ...
            try:
                async for msg in user_app.get_chat_history(chat["id"], limit=1):
                    ...
                    if msg.id > last_id[chat["id"]]:
                        ...
                        # 1. فلتر الاستثناءات السريع (لتوفير الـ API)
                        if any(ex in text_c for ex in EXCLUDED): continue

                        # 2. منطق المسارات والنية
                        ...
                        if is_route or has_intent:
                            # 3. التأكيد النهائي بالذكاء الاصطناعي
