...
genai.configure(api_key="AIzaSyBifh9-Lku7bbkp_PKiEy-SZgKAlmMCvV0")
...
async def ai_analyze_message(text):
    # 1. قائمة الكلمات الإخبارية والدردشة (الرفض الفوري)
    # أضفنا كلمات مثل "الموقع" و "الطريق" التي يستخدمها السائقون عادةً عند التنفيذ
    STORY_KEYWORDS = [
        "جاني مشوار", "رحت", "وصلت", "كنت في", "خلصت مشوار", "كنت بـ",
        "الموقع أرسل", "دربك سمح", "الله يرزقك", "الطريق زحمة", "تم الارسال"
    ]
    if any(word in text for word in STORY_KEYWORDS):
        return False

    # 2. استبعاد إعلانات السائقين (DRIVER_KEYWORDS مفترض تعريفها مسبقاً)
    # تشمل: (موجود، متاح، سيارة نظيفة، توصيل مشاوير، فحص دوري)
    if any(word in text for word in DRIVER_KEYWORDS):
        return False

    # 3. الـ Prompt المطور (المحقق الذكي)
    # ركزنا هنا على "نية الطلب" و "نوع الحمولة" (ركاب أو أغراض)
    prompt = f"""
    حلل نية المرسل في النص التالي: "{text}"
    
    المهمة: تحديد ما إذا كان المرسل (زبون) يطلب خدمة توصيل (أشخاص أو أغراض) في الوقت الحالي.

    أجب بـ (YES) في الحالات التالية فقط:
    1. طلب مشوار لشخص (مثلاً: "ابي مشوار"، "من يوصلني"، "في أحد يروح لـ").
    2. طلب توصيل غرض أو طرد (مثلاً: "من يوصل أغراض"، "عندي طلب من مطعم"، "في أحد يوصل من صيدلية").
    3. استفسار عن توفر سائق لوجهة محددة الآن.

    أجب بـ (NO) في الحالات التالية:
    1. إذا كان المرسل (سائق) يعرض خدماته (مثلاً: "أنا موجود"، "توصيل للمطار"، "سيارة متاحة").
    2. إذا كان المرسل يخبر عن حالة (مثلاً: "وصلت العميل"، "الطريق زحمة").
    3. إذا كان مجرد شكر أو دعاء أو دردشة خارج موضوع الطلب.

    الرد بكلمة واحدة فقط: YES أو NO.
    """

    try:
        # تشغيل طلب الـ AI في executor لعدم تجميد الـ Loop
        loop = asyncio.get_event_loop()
        response = await loop.run_in_executor(None, lambda: ai_model.generate_content(prompt))
        
        # تنظيف الرد (لأن الـ AI قد يضيف نقاط أو فراغات)
        answer = response.text.strip().upper()
        
        # التأكد من أن الرد يحتوي على YES فقط وليس NO (مثل: NO, because...)
        is_request = "YES" in answer and "NO" not in answer
        
        print(f"🧠 تحليل AI للنص [{text[:30]}...]: القرار هو {answer}")
        
        return is_request

    except Exception as e:
        print(f"⚠️ خطأ في معالجة الذكاء الاصطناعي: {e}")
        return False
